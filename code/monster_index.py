...
class MonsterIndex:
    ...
    def display_main(self,dt):
        ...
        energy_rect = pygame.FRect((0,0), (bar_data['width'], bar_data['height'])).move_to(midtop = (bar_data['right_side'], bar_data['top']))
        draw_bar(self.display_surface, energy_rect, monster.energy , monster.get_stat('max_energy'),COLORS['blue'], COLORS['black'], 2)
        ...
        for index, (stat, value) in enumerate(monster_stats.items()):
            ...
            ability_text = self.fonts['regular'].render('Ability', False, COLORS['white'])
            ability_text_rect = ability_text.get_frect(bottomleft = ability_rect.topleft)
            self.display_surface.blit(ability_text, ability_text_rect)
